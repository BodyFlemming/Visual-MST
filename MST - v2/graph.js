class Graph {
    ...
    show() 
    {
        ...
        { 
            ...
            for (let k = 0; k < u.adj.length; ++k) 
            {
                ...
                // Makes sure to not draw ontop of a already drawn line OR a MST-line 
                if (v.id < i || this.vertices[i].parent == v.id || this.vertices[v.id].parent == i) continue;
                stroke(255, map(this.weight[i][v.id], width*0.8, 0, 1, 60)); // Fades the line based on distance between vertices
    ...
    // Shamelessly stolen from our excercises and modified of course
    primMST() {
